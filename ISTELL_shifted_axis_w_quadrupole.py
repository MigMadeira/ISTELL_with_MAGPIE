...
from simsopt.field import BiotSavart, DipoleField, Current, coils_via_symmetries, Coil
from simsopt.geo import PermanentMagnetGrid, SurfaceRZFourier, create_equally_spaced_curves, curves_to_vtk, CurveRZFourier
...
famus_filename = 'grids/ISTELL_shifted_axis/ISTELL_1cm_cubes_shifted_radial_extent=02725m_nfp=2_w_tor_removal.focus'
...
OUT_DIR = 'ISTELL_shifted_axis/no_diagnostics_w_quadripole/'
...
base_currents = [Current(1.0) * 2e3 for i in range(ncoils)]
...
for i in range(len(quadrupole_curves)):
    #fix degrees of freedom of the quadrupole to guarantee it is circular.
    quadrupole_curves[i].fix_all()
    quadrupole_curves[i].unfix("x0")
    
    #add the quadrupole to the remaining coils
    coils.append(Coil(quadrupole_curves[i], quadrupole_currents[i]))
